...
get/post/put/del 제공, res.ok 체크 + JSON/text 자동 파싱
...
GET /alerts?page=1&size=... 알림 목록(프론트 0-based → 백엔드 1-based 변환 포함)
...
fetchDashboardSummary() 호출해서 보호상태 요약을 summary로 제공

UseLogs.js

로그 목록 상태/필터/페이지 관리

/logs 실패 시 /logs/recent로 fallback하는 로직 포함
...
예외(화이트리스트) 규칙 목록/추가/삭제 + SettingExceptionsPage에서 쓰기 좋게 별칭 제공
...
보호상태 표시, 감시 시작/중지, 스캔 시작/일시정지/진행률, 감시폴더 간단 관리, 최근 이벤트 표시

사용 API: DashboardApi, WatcherApi, ScanApi, useLogs, useWatchedFolders

logs/LogsPage.jsx: 로그 조회/검색/상세/삭제/내보내기

LogFilterBar, LogTable, LogDetailModal 조합

사용 API: LogsApi, 훅: useLogs

notifications/NotificationPage.jsx: 알림 목록 + 페이지네이션/필터

notifications/NotificationDetailPage.jsx: 알림 상세(라우팅 state가 없으면 서버에서 id로 재조회)
...
서버 실패 시 localStorage로 fallback 하는 방어 로직 포함
...
→ 지금은 프로젝트에서 “공통 컴포넌트 자리만 잡아둔 상태”라 실제 구현이 필요하거나, 안 쓰면 삭제해도 됨.
